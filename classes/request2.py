...
def _create_response(response):
    ...
    if sys.version_info.major == 3:
        ...
        for header in headers:
            ...
            key = header.split(':')[0].lower().strip()
...
class RedirectHandler(urllib2.HTTPRedirectHandler):
    ...
    def http_error_302(self, req, fp, code, msg, headers):
        if 'location' in headers:
            ...
            new_url = urlparse(headers['location'])
...
class Response:
    ...
    def set_body(self, body):
        # define encoding method by python version
        if sys.version_info.major == 3:
            enc = str
        elif sys.version_info.major == 2:
            enc = unicode
        ...
        if content_type not in self.headers:
            self.body = enc(body, errors='replace')
        else:
            ...
            if content_types['charset'] is not None:
                self.body = enc(body, content_types['charset'], errors='replace')
            elif content_types['text']:
                self.body = enc(body, 'ISO-8859-1', errors='replace')
            else:
                self.body = enc(body, errors='replace')
...
class Requester:
    def __init__(self, options, data):
        ...
        self.url_data = urlparse(options['url'])
        if options['prefix']:
